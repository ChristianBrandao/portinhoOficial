...
const PrizeDetail = () => {
    ...
    const [selectedPrice, setSelectedPrice] = useState(0.20); // Initial price for 10 titles at 0.02 each
    const [isCheckoutOpen, setIsCheckoutOpen] = useState(false);
    const [instantWinner, setInstantWinner] = useState(null);

    React.useEffect(() => {
        if (prize) {
            setQuantity(10); // Default to 10 titles
            setSelectedPrice(10 * prize.pricePerTicket); // Calculate initial price based on new pricePerTicket
        }
    }, [prize]);
                ...
                <main className="w-full max-w-2xl px-4 py-8">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                        ...
                        <div className="flex justify-center items-center gap-2 mb-6">
                            ...
                            <span className="bg-black text-white font-bold text-lg px-4 py-1 rounded">R$ {prize.pricePerTicket.toFixed(2).replace('.',',')}</span>
                        </div>

                        {/* Meus Títulos Section */}
                        ...
                        <div className="bg-gray-800 rounded-lg shadow-sm p-4 mb-6">
                            ...
                            <div className="mt-4 space-y-2">
                                {winners.map((winner) => (
                                    <div key={winner.id} className={`flex items-center justify-between p-2 rounded-lg text-sm ${winner.awarded ? 'bg-black text-white' : 'bg-gray-700 text-gray-300'}`}>
                                        <span className={`font-bold px-3 py-1 rounded-full ${winner.awarded ? 'bg-black border border-white' : 'bg-gray-600'}`}>{winner.id}</span>
                                        <span className="flex-1 text-center font-semibold">{winner.prize}</span>
                                        <div className="flex items-center gap-1 font-bold">
                                            <span>{winner.name}</span>
                                            {winner.awarded && <Trophy size={16} className="text-yellow-400" />}
                                        </div>
                                    </div>
                                ))}
                            </div>
