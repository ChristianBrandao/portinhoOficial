...
const CheckoutDialog = ({ isOpen, setIsOpen, quantity, totalPrice, productName, onPaymentSuccess }) => {
    ...
    useEffect(() => {
        if (step === 3 && paymentData?.reservationId) {
            paymentCheckInterval.current = setInterval(async () => {
                const result = await checkPaymentStatus(paymentData.reservationId, user);
    ...
    }, [step, paymentData, navigate, user]);
    ...
    const handleReserve = async () => {
        setIsLoading(true);
        try {
            const orderDetails = { userId: user.id, quantity, totalPrice, productName };
            const data = await reserveNumbersAndCreatePayment(orderDetails);
            setPaymentData(data);
            setStep(3);
            setTimer(300);
        } catch (error) {
            toast({ title: 'Erro na Reserva', description: 'Não foi possível criar o pagamento. Tente novamente.', variant: 'destructive' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleRegisterRedirect = () => {
        handleClose();
        navigate('/cadastro', { state: { phone: phone } });
    };
                    ...
                    <motion.div key="step3" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50, transition: { duration: 0.2 } }}>
                        ...
                        <div className="space-y-4 pt-4 text-center">
                            ...
                            <div className="p-4 bg-white rounded-lg inline-block">
                                {paymentData && <QRCode value={paymentData.qrCode} size={200} />}
                            </div>
